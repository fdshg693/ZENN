...
Copilot CLI は便利ですが、単発で [`-p` 実行](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options) するだけだと、VS Code 上の Copilot Chat / Agent を使う場合より明確な優位があるとは言いにくいです。
...
この基礎編の続きとして、[後編「Copilot CLI をスクリプトで活用する: Hooks による継続実行と YAML ワークフロー化」](./zenn-copilot-cli-lv3) では、`sessionEnd` を起点にした follow-up 実行と、YAML + Python によるワークフローオーケストレーションを扱います。

こちらの記事では、その前提になる [`-p`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options)、順次実行・並列実行、[`--resume`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options)、Hooks での `session_id` 取得までを押さえます。後編は「単発実行の応用」ではなく、**セッションのライフサイクルや複数タスクの制御をどう設計するか**に踏み込む位置づけです。
...
Copilot CLI をプログラムから扱うとき、まず押さえておきたいのが次の形です。ここでは [`-p`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options) と [`--silent`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options) を使って、スクリプトから扱いやすい最小形に寄せています。
...
- [`--continue`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options): 直前のセッションを再開
- [`--resume=<SESSION-ID>`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options): 指定したセッションを再開
...
公式ドキュメントでは、[`GitHub Copilot CLI configuration directory`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-config-dir-reference) の [`session-state/`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-config-dir-reference#automatically-managed-files) の説明で、セッション履歴がここに保存され、[`--resume`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options) / [`--continue`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#command-line-options) に利用されることが案内されています。
...
より素直なのは、Copilot CLI の [Hooks](https://docs.github.com/en/enterprise-cloud@latest/copilot/how-tos/copilot-cli/customize-copilot/use-hooks) を使う方法です。

公式ドキュメントによると、リポジトリの [`.github/hooks/*.json`](https://docs.github.com/en/enterprise-cloud@latest/copilot/how-tos/copilot-cli/customize-copilot/use-hooks#creating-a-hook-in-a-repository-on-github) に Hook 定義を置くと、CLI が自動で読み込みます。さらに [`SessionStart`](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference#sessionstart--sessionstart) イベントを PascalCase で定義すると、VS Code 互換の snake_case ペイロードが渡され、`session_id` をそのまま取り出せます。
...
- [Copilot CLI command reference](https://docs.github.com/en/enterprise-cloud@latest/copilot/reference/copilot-cli-reference/cli-command-reference)
  - `--available-tools`
  - `--disable-builtin-mcps`
  - `-p`
  - `--continue`
