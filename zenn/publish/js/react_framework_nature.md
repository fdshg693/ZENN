...
それでも、同僚や後輩に「なぜReactなのか」「Reactは何を解いて、何を代償にしているのか」と聞かれて、**短く自分の言葉で説明できるか**と問われると即答に詰まることが多い。

本記事はReactの文法を一切説明しません。**フレームワーク全体の性質**として、

1. ネイティブJSが詰んだのはどこで、Reactはその問題をどう再定義したのか
2. 代わりにReactは何を自分の制約として引き受け、利用者に何を強いたのか
3. その性質から自然に導かれる「勝ち筋」と「地雷」は何か
4. どのユースケースでReactが有利/不利になるのか

を、Dan Abramov の "React as a UI runtime" や公式 Docs を根拠に整理します。日常でReactを書いている人が、**原理のレベルで判断できる軸**を手に入れることを目的とします。
...
小さなフォームならこれで十分機能します。公式も同じことを述べています:
...
> 訳: 単独の例ではUIを命令的に操作しても十分機能するが、より複雑なシステムでは管理の難しさが指数的に膨らむ。

「指数的に難しくなる (exponentially more difficult)」と言い切っている点が重要です。
ここで破綻しているのは単なる行数ではなく、**状態遷移の組み合わせ数**と、**DOMとの整合性を人間が維持するコスト**です。相互作用が N 個あると、「ボタンが押せるはずなのに disabled のままだ」のようなズレのパターン数が概ね O(N²) 以上のオーダーに膨らみます。

Reactが最初に行った設計上の選択は、この問題の形を書き換えたことにあります。
...
> 訳: Reactでは UI を直接操作するのではなく、何を表示したいかを宣言する。UI をどう更新するかは React が判断する。

つまり、「状態 → ビュー」の**写像を一方向の純関数にする**。DOMとの同期という面倒な後半戦は、Reactというランタイムに引き受けさせる。これがReactの基本姿勢です。

---

## 2. Reactは「DOMライブラリ」ではなく「UIランタイム」

ここで、Reactを「Virtual DOMのライブラリ」として捉えていると理解の精度が落ちます。より正確なモデルは Dan Abramov の言う **UI runtime** です。
...
> 訳: Reactのプログラムは時間とともに変化しうるツリーを出力する。それは DOMツリーであっても、iOS の階層であっても、PDFプリミティブのツリーであっても、JSONオブジェクトであっても構わない。…抽象的に言えば、Reactは、ユーザー操作・ネットワーク応答・タイマーといった外部イベントに対して、複雑な宿主ツリーを予測可能に操作するプログラムを書くための仕組みである。

ここから引ける性質は3つあります。

### 2.1 宿主ツリーは交換可能

React自体は DOM に依存していません。DOM・iOS・PDF・JSON、どれでも「ツリー状の宿主」があればレンダラを差し替えて動作します。これが React Native, React Three Fiber, react-pdf, Ink(CLI)などの基盤です。
Web開発の文脈ではDOMが大半を占めますが、**DOMでの挙動は「特定のレンダラ実装の詳細」にすぎない**という視点を持つと、パフォーマンス議論の筋が通しやすくなります。

### 2.2 Reactは外部イベントに反応する「調整役」

Reactは coordinator として振る舞います。これは「疎結合な小さなモジュールの集合」としては実装できない、とReactチーム自身が明言しています:
...
> 訳: Reactは小さく疎結合なモジュールに分割することはできない。役割を果たすには何かが調整役(coordinator)を担う必要があり、Reactとはまさにそれである。

「複雑性はReact側が吸収する (Absorb the Complexity)」という原則も対をなしています。つまり、**Reactの中央集権性は副作用ではなく本質**です。軽量で疎結合なライブラリを必要とするなら、そもそも別の設計(例: signal 系)を選ぶことになります。
...
> 訳: 状態がクロージャやコンビネータの中に「閉じ込められず」、Reactが直接アクセスできることは明示的な設計目標である。props と state を引数とする同期的な render 関数を採用することで、デバッグは推測作業ではなく、退屈ではあるが有限手数で完了する手続きに変わる。
...
DevToolsでコンポーネントツリーを辿ってバグ原因の state にたどり着くという体験は、この設計制約の直接の帰結です。
...
> 訳: 注意点として、Reactは時に反応的すぎる、つまり過剰に再レンダーすることがある。JavaScript には2つのオブジェクトや配列の等価性を安価に比較する手段がないため、毎レンダーで新規オブジェクトや配列を生成すると、Reactは厳密に必要な以上の仕事をしてしまう。結果として、開発者は変更に過剰反応しないようコンポーネントを明示的にメモ化する必要がある。
...
**実務的な含意**: props の「形」は Reactの等価性モデルを前提に設計するべきものです。巨大オブジェクトやインラインで作られた配列/関数を無造作にバラ撒くと、memoization は容易に無効化されます。React Compiler がこれを自動化しようとしているのは、この問題を「言語として」解きにいく試みです。

### 3.2 Effectは「ライフサイクル」ではなく「同期プリミティブ」

ここは、すでに書ける人でも原理に立ち返ると整理が難しい箇所です。Reactの公式立場は一貫しており、**Effect は外部システムとの同期のためにある**と位置づけています。
...
> 訳: 外部システムとの同期には Effect が必要である。…render の中で計算できることであれば、Effect は要らない。
...
> 訳: Effect の中で state を即座にセットすると、React は render サイクル全体をやり直さざるを得なくなる。…これにより余計な render パスが発生する。render 中に直接データを変換するか、props から state を導出していれば回避できたはずの追加レンダーである。
...
> 訳: useEffect API そのものの代替や利便ラッパーとして振る舞う、独自の「ライフサイクル」Hook を作って使うのは避けるべきである。…たとえば useMount のような Hook は roomId や serverUrl の変化に「反応」しないが、linter は useEffect の直接呼び出ししか検査しないため警告してくれない。

ポイントは、依存配列の検査は useEffect の**直接の呼び出し**でしか効かないこと。`useMount` でラップされた瞬間、「reactivity」という Reactの中核メカニズムが見えなくなる。ゆえに公式が推奨しないのです。これは単なる好みではなく、**Reactの整合性モデルに直接響く**話です。
...
> 訳: Reactは、コンポーネントが UI ツリー上の同じ位置でレンダーされ続けている間、その state を保持する。位置から取り除かれたり、同じ位置に別のコンポーネントがレンダーされたりすると、Reactはその state を破棄する。…state は JSX タグに紐づいているのではなく、その JSX を置いたツリー位置に紐づいている。

これが意識に上がっていないと、次のようなコードが直感に反して見えます。

```jsx
{showA ? <Counter /> : <Counter />}
```

直感的にはコンポーネントが切り替わっているのだから state はリセットされそうに見えますが、**同じ位置に同じ型**が置かれているので、Reactから見るとこれは同一のインスタンスであり、state は保持されます。
逆に、「別のエンティティを表示している」ことを表現したい場合は、`key` を変える必要があります。

```jsx
<Profile key={userId} />
```

これでReactはsubtreeを別物と認識し、**丸ごと作り直します**。公式ドキュメントはこれを propsからの state リセットの**推奨手段**として位置づけています。`getDerivedStateFromProps` のような煩雑な同期処理は、概ねこの `key` 操作で代替できます。
...
真面目に木差分を解くと O(n³) なので、Reactは「型が同じなら同じ、違えば破棄・再生成」「リストはkeyで追跡」というヒューリスティックを置いて O(n) に抑えています。そのために、**利用者が key を安定・予測可能・一意に保つ義務**を負います。`Math.random()` や機械的な index を key に使うと、並び替えや挿入で state と DOM インスタンスの対応が崩れます:
...
> 訳: 安定しない key(`Math.random()` のようなもの)を使うと、多数のコンポーネントインスタンスや DOM ノードが不必要に再生成され、パフォーマンス劣化や子コンポーネントの state 喪失を招く。

`key` を「Reactの warning を抑えるための回避手段」と捉えている例が散見されますが、実態はほぼ逆で、**key は subtree の identity を利用者が宣言する唯一の公式プリミティブ**です。
...
> 訳: ゼロから始めるのは Reactを使い始める簡単な方法だが、注意すべき大きなトレードオフがある。この道を進むことは、しばしば自分専用のフレームワークを自作するのと同じ意味になる。

つまりReactは、UIツリーの差分更新と状態管理という**狭く深い問題**に特化したランタイムであり、アプリケーションの残り半分(データ取得・配信・画面遷移)は意図的にスコープ外です。Next.js や React Router を選ぶのは、このスコープ外を誰に任せるかの判断です。
...
1. **"source of truth" は1箇所にし、残りは render で派生する**
   選択中のアイテムを state に持たずに、`selectedId` だけを保持して render で `items.find` する。Effect + setState での派生同期はほぼ全てこれで解消される。

2. **状態の identity は `key` で明示的に切る**
   「別のものを表示している」ことを表現したいときは、`getDerivedStateFromProps` ではなく `key` を変える。これがReactネイティブな identity 宣言手段。

3. **render は純関数として守る**
   副作用は event handler か Effect に寄せる。render 中にグローバル可変状態を書き換えない。Strict Modeの二重呼び出しは、これを機械的に検出するためのものである。

4. **Effect は外部世界との同期専用**
   外部サブスクリプション、サードパーティウィジェット、ブラウザAPIとの接続のみに用いる。**「X になったら Y の state を更新する」の9割は Effect ではなく render または event handler に書くべき**。

5. **参照同一性を設計の一部として扱う**
   props に毎レンダー新規オブジェクトを生成しない。依存配列にオブジェクト/関数を入れない。分解してプリミティブに直す、またはオブジェクトは Effect の中で生成する:
   ...
   > 訳: 可能な限り、Effect の依存にはオブジェクトや関数を入れないようにする。代わりに、それらをコンポーネントの外に出すか、Effect の内側に移すか、プリミティブな値を取り出して使う。
...
- render の最中にグローバル可変変数や ref を書き換える(render の純粋性と time-slicing を壊す)
- key に `Math.random()` や配列 index を機械的に使う(並び替え・挿入で state と DOM の対応が崩れる)
- `memo` を「とりあえず貼る」(props が毎回新規オブジェクトなら効かず、比較コストだけが残る)
- 巨大な object を context で流し、全 consumer を連鎖再レンダーさせる
- Reactが管理する DOM を、Reactの外から直接操作して内部状態との不整合を作る(reconciliation 後の DOM 復元で必ずどこかで破綻する)

これらが「なぜダメか」を**機能単位**ではなく**Reactの原理単位**で説明できることが重要です。
...
- 1フレーム精度のアニメーション・物理シミュレーション
  — renderサイクルを避けて DOM/Canvas を直接触る方が Reactの抽象と正面から競合せずに済む(Reactの外に逃がすための `useSyncExternalStore` や ref の使い方は用意されている)
- 極端に制約された配信環境で、ランタイム本体のサイズが予算に収まらないケース
  — Preact や signal 系、あるいは islands アーキテクチャの方が適合する

ここで重要なのは、「Reactが遅い/重い」ではなく、**Reactが解こうとしている問題と、そのユースケースが解いてほしい問題が違う**、という捉え方です。
...
- 日々書いているパターン(`key` でリセット、`useMemo` で参照安定化、render 中の派生計算、…)が**すべて同じ原理の帰結**として見える
- 迷ったときに「Reactはこの問題をどう定義したか?」に立ち返って判断できる
- Svelte や Solid、signal 系、islands との比較も「別の賭けをしている」という構造として読める

この視点を持つだけでも、同僚に「なぜReactなのか?」と問われたときの答えは明確に変わるはずです。
