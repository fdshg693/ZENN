...
Solid を触ったことがあると、「React に似ているが destructure ができない」「コンポーネントが一度しか走らない」といった粗い印象に留まりがちです。
それでも、同僚や後輩に「なぜ Solid なのか」「Solid は何を解いて、何を代償にしているのか」と聞かれて、**短く自分の言葉で説明できるか**と問われると即答に詰まります。

本記事はSolidの文法を一切説明しません。同シリーズで React を「UIランタイム」、Vue を「コンパイラ付きリアクティブランタイム」として捉え直したのと同じ視点で、Solid を見直します。**フレームワーク全体の性質**として、
...
を、公式 Docs と Ryan Carniato(Solid 作者)の発信を根拠に整理します。日常で Solid を書いている人が、**原理のレベルで判断できる軸**を手に入れることを目的とします。
...
> 訳: Solid アプリのインタラクティブ性は reactivity が支えている。これはデータや状態の変化に自動で応答するシステムの能力を指すパラダイムである。Solid では reactivity が設計の基礎であり、これによりアプリは常に背後のデータと整合した状態に保たれる。
...
> 訳: コンポーネントは再実行されない。再実行されるのは利用しているプリミティブと JSX 式だけである。React から来た人にとっての stale closure や Hook Rules は存在しない。…React・Vue・Svelte と違い、Solid は真に粒度の細かい更新を行う。つまりコンポーネントは初回実行後、実質的に「消える」ということである。
...
> 訳: コンポーネントは他の関数と同じく、一度しか実行されない。return 文の外で signal にアクセスすると、初期化時には実行されるが、その後 signal が更新されても更新はトリガされない。

つまりコンポーネント関数は**セットアップ**です。1回走って、JSX から生成されたDOMとリアクティブプリミティブを構築し、自分は消える。残ったプリミティブが signal の変化に反応して DOM を直接更新する、というモデルです。なお「コンポーネント関数は1回で終わる」だけで、そこから生成された所有スコープ(reactive primitives)は生き続け、ツリーから取り除かれるタイミングで破棄されます。後述する `onCleanup` はこのスコープ破棄時に走ります。

React 由来の「stale closure」「Hook Rules」「依存配列の整合性」という一連の負債は、この設計では**そもそも発生しません**。代わりに、「コンポーネント本体で値を変数に束縛すると、それは初回コピーで凍結される」という別のコストを負うことになります。
...
> 訳: Solid では、変更が必要な対象の属性だけに更新が反映され、より広範な(時に不要な)更新を避ける。対照的に React は、単一属性の変更に対してもコンポーネント全体を再実行するため、効率が劣ることがある。
...
> 訳: ではこれをどう実現しているか。すべての動的 props を単純にレイジー評価しているだけである。
...
こうしておくと、`a()` の実行は**値が最終的に消費される場所**(JSX式 or effect内)まで遅延されます。結果として、コンポーネント境界を越えても reactivity がつながったままになります。Ryan の言葉で言えば:
...
> 訳: 一見、単純なバインディングに見えるものが、実際にはビューコード全体を貫くリアクティブなストリームを生成し、コンポーネント境界を越えてピンポイントの精度で更新を波及させている。
...
Reactが利用者に4つの義務(over-reactivity、Effect は同期プリミティブ、state identity はツリー位置、単体ランタイムの限界)を引き受けさせるのと同様に、Solid の賭けも代償を伴います。
...
> 訳: tracking scope は他の Solid プリミティブである `createEffect` または `createMemo` によって作られる。両関数は、その内部でアクセスされた signal を購読し、依存関係を確立する。

重要なのは、**コンポーネント関数本体は tracking scope ではない**ということです。したがって、本体で signal を読むと「初回のスナップショット」になります。なお、JSX 内の式はコンパイラによって tracking scope として登録されるため、signal の変化に応じて再評価されます。
...
これは Solid 固有の、最も誤りやすい制約です。公式はここを明確に禁止しています:
...
> 訳: 他のいくつかのフレームワークと異なり、コンポーネントの `props` に対してオブジェクト分割代入を使うことはできない。
...
> 訳: Solid では props の分割代入は推奨されない。反応性を壊しうるためである。代わりに props オブジェクトから直接アクセスするか、関数で包んで常に最新の値が取れるようにすべきである。

次のコードは Reactなら何も考えずに書きますが、Solid では**反応性が失われます**:
...
> 訳: props オブジェクトは readonly であり、リアクティブなプロパティは Object の getter で包まれている。…一般に、Solid のプリミティブや JSX の外で props オブジェクトのプロパティにアクセスすると反応性が失われうる。これは分割代入だけでなく、spread や `Object.assign` のような関数にも当てはまる。
...
ここは同シリーズの React / Vue 編と**ほぼ同じ構造**の議論であり、原理は横並びです。公式の立場:
...
> 訳: Effects は主として、reactive システムに書き込まない副作用を扱うために用意されている。effect 内で signal をセットするのは避けるべきである。注意深く扱わないと追加の再レンダーや無限ループに陥りうる。代わりに、他の reactive 値に依存する新しい値を計算したい場合は `createMemo` の利用が推奨される。
...
細かい差分として、Solid は**依存配列を持ちません**。tracking scope 内で実際に読まれた signal が自動的に依存になります。Vue の `watchEffect` に近い挙動で、React の依存配列 lint はありません。代わりに「値を変数に束縛すると追跡されない」というスコープ側の制約で全体のバランスを取っています。
...
**何が嬉しいのか**: React の `key={index}` は「大抵は不適切だが、場面によっては妥当」という曖昧な選択になりがちでした。Solid は**どちらの戦略を取っているか**がコンポーネント名で表明されます。選択ミスは型では防げませんが、レビューで可視化されます。
...
> 訳: SolidStart は SolidJS の first-party なプロジェクトスターター/フレームワークであり、その規模感を超える(身の丈以上の)機能を提供し、Solid アプリをデプロイするための first-class な方法を与える。
...
ただし、エコシステムの厚みは React や Vue には及びません。React Native 相当、Nuxt のモジュール生態系、Next.js の豊富な統合ライブラリといったものは一桁以上少ない。**Solid を選ぶことは「ランタイムの性質を取りに行く」判断**であり、エコシステム全体を踏まえた選定になります。
...
1. **派生は `createMemo` / 派生 signal、同期だけ `createEffect`**
   「Aが変わったらBを更新する」のほぼ全ては `createMemo` で書ける。`createEffect` の中で `setSignal` する前に、「`createMemo` で表現できないか」を先に自問する。
...
これらが「なぜダメか」を**機能単位**ではなく**Solid の原理単位**で説明できることが重要です。
...
- チームに「reactive scope / props の扱い」を教える余裕がない現場 — Reactと外見が似ているがゆえに、destructure 1 回で反応性が失われるという誤りが頻発しやすい
...
- 日々書いているパターン(destructure 禁止、`<Show>` で分岐、`createMemo` で派生、`onMount` で初期化、…)が**すべて同じ原理の帰結**として見える
- 迷ったときに「Solid はこの問題をどう定義したか?」に立ち返って判断できる
- React や Vue、Svelte 5 runes、Vue Vapor、TC39 Signals との比較も「**別の賭けをしている**」という構造で読める

姉妹記事の React / Vue と並べるとさらに見通しが良くなります。**同じ「状態空間の爆発」という問題**に対して、
