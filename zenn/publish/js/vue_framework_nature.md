...
それでも、同僚や後輩に「なぜVueなのか」「Vueは何を解いて、何を代償にしているのか」と聞かれて、**短く自分の言葉で説明できるか**と問われると即答に詰まることが多い。

本記事はVueの文法を一切説明しません。同シリーズの「Reactを"UIランタイム"として捉え直す」と対をなす位置づけで、**フレームワーク全体の性質**として、
...
を、公式 Docs と Vue チームの発信を根拠に整理します。日常でVueを書いている人が、**原理のレベルで判断できる軸**を手に入れることを目的とします。
...
小さなフォームなら成立しますが、相互作用が N 個あると整合性違反のパターン数が概ね O(N²) 以上のオーダーに膨らみます。
...
> 訳: 宣言的レンダリング: Vue は標準 HTML をテンプレート構文で拡張し、JavaScript の状態に基づいて HTML 出力を宣言的に記述できるようにする。リアクティビティ: Vue は JavaScript の状態変化を自動的に追跡し、変化が起きたときに効率的に DOM を更新する。

つまり、**「状態 → ビュー」の写像を宣言的に書き、DOM 同期はフレームワークに任せる**。この方向まではReactとまったく同じです。
...
Vueを「テンプレート言語を持つReact風のもの」として捉えていると理解の精度が下がります。より正確なモデルは **compiler-informed reactive runtime** です。公式はこれを明示しています:
...
> 訳: Vue ではフレームワークがコンパイラとランタイムの両方を制御している。これにより、密結合なレンダラだけが活用できるコンパイル時最適化を多数実装できる。…我々はこのハイブリッド方式を Compiler-Informed Virtual DOM と呼んでいる。
...
> 訳: Vue 2 はブラウザサポートの制約により getter/setter のみを使っていた。Vue 3 では、リアクティブオブジェクトには Proxy、ref には getter/setter を使う。
...
> 訳: 本質的に、signals は Vue の ref と同じ種類のリアクティビティプリミティブである。アクセス時に依存追跡し、変更時に副作用をトリガする値コンテナである。
...
> 訳: コンパイラがテンプレートを静的解析し、生成コードにヒントを残すことで、ランタイムは可能な限りショートカットを取れる。
...
> 訳: 真にリアクティブで、コンパイラ最適化されたレンダリングシステム。手動最適化はほぼ不要。

Reactの `useMemo` / `useCallback` / `memo` を利用者に強いる姿勢と真っ向から対照的です。「自動でよしなに」を runtime の設計で押し切るのがVueのスタンスです。
...
> 訳: ユースケースに応じて、Vue は以下のように異なる形で使える: ビルドステップなしで静的 HTML を強化する/Web Components として任意のページに埋め込む/シングルページアプリケーション(SPA)/フルスタック・サーバーサイドレンダリング(SSR)/Jamstack・静的サイト生成(SSG)/デスクトップ、モバイル、WebGL、ターミナルまで対象とする。
...
- `ref()`: 任意の値を `.value` で包む。プリミティブにも適用可能。
...
> 訳: `reactive()` には以下の制約がある。(1) 値の型が限定される: オブジェクト型にしか使えず、string・number・boolean などプリミティブ型は保持できない。(2) オブジェクト全体を置き換えられない: Vue の反応性追跡はプロパティアクセスを介して動作するため、同じ参照を保ち続ける必要がある。reactive オブジェクトを「置き換える」と、最初の参照に対する反応性の接続が失われる。
...
> 訳: `toRefs` は、composable 関数から reactive オブジェクトを返すときに有用。呼び出し側のコンポーネントが、反応性を失わずに分割代入や spread で受け取れるようになる。

公式ドキュメント自身が「composable は `reactive` ではなく `ref` で返すのを推奨」と書いています:
...
> 訳: composable では `reactive()` ではなく `ref()` のみを使っていることに気づくはず。推奨される慣例は、composable は常に「複数の ref を含む素の(非リアクティブな)オブジェクト」を返すことである。これによりコンポーネント側で反応性を保ったまま分割代入できる。
...
> 訳: reactive オブジェクトは分割代入すると反応性を失いやすい。一方、ref は至るところで `.value` を書くのが煩わしい。型システムを使っていないと `.value` の付け忘れにも気づきにくい。
...
> 訳: 反応するのは proxy だけであり、元のオブジェクトを変更しても更新はトリガされない。したがって Vue の反応性システムを扱うベストプラクティスは、状態を必ず proxy 化された版で扱うことである。
...
> 訳: 単純にリアクティブにすべきでない値もある(複雑なサードパーティのクラスインスタンスや Vue コンポーネントオブジェクトなど)。Proxy 変換をスキップすれば、不変なデータソースで大量リストを描画する際のパフォーマンス向上にもつながる。ただしこの raw への opt-out はルートレベルにしか効かない高度な機能であり、ネストされた未マーク raw オブジェクトを reactive オブジェクトに代入してから再アクセスすると、proxy 化された版が返ってくる。これは **identity hazard** につながりうる — 同一オブジェクトの raw 版と proxy 版を同時に使ってしまい、オブジェクト identity に依存した処理が壊れる、というケースである。

**実務的な含意**: Vue で「reactive に何を入れるか」は単なるスタイルではなく設計判断です。**「観測可能にすべきでないもの」を誤って reactive に入れると、identity 依存のロジックが壊れる**。これはReactの「毎レンダー新規オブジェクトを生成すると memo が効かない」と同じ性質を持つ、runtime 由来の制約です。
...
> 訳: reactive effect は Vue の反応性システムの一部であり、ある関数の依存を追跡し、依存の値が変わったらその関数を再実行する仕組みを指す。`watchEffect()` は effect を直接的に作る最も素直な方法。Vue の他の部分(コンポーネントの再描画、`computed()`、`watch()` など)も内部的に effect を使っている。
...
> 訳: reactive effect を使って ref を変更するのはあまり良いユースケースではない。実際、computed プロパティを使う方が宣言的に書ける。
...
> 訳: `watch` は明示的に指定した source のみを追跡する。一方 `watchEffect` は依存追跡と副作用を1段階に統合し、同期実行中にアクセスしたすべての reactive プロパティを自動的に追跡する。便利で記述も短くなるが、反応性の依存は明示性が低くなる。
...
> 訳: `watchEffect` は同期実行中の依存だけを追跡する。非同期コールバックと併用した場合、最初の `await` 以前にアクセスしたプロパティしか追跡されない。
...
> 訳: Vue が `v-for` で描画されたリストを更新するとき、デフォルトでは "in-place patch" 戦略を使う。データ項目の順序が変わっても、DOM 要素を順序に合わせて移動させるのではなく、各要素をその位置のまま patch して、その index で描画すべき内容を反映させる。このデフォルトは効率的だが、**リストの描画結果が子コンポーネントの state や一時的な DOM 状態(フォームの入力値など)に依存しない場合にのみ適している**。
...
> 訳: key がない場合、Vue は要素の移動を最小化するアルゴリズムを用い、同じ型の要素を可能な限りその場で patch/再利用しようとする。key がある場合は、key の順序変化に基づいて要素を並び替え、もはや存在しない key を持つ要素は必ず削除・破棄される。…また、**要素/コンポーネントを再利用するのではなく強制的に置き換える** 用途にも使える。

Reactで `<Profile key={userId} />` と書くと subtree を丸ごと作り直すのと、まったく同じ機能が Vue にもあります。「別のエンティティを表示している」ことを宣言する公式プリミティブ、という位置付けも同一です。

index を key に使わない、という教訓もそのまま適用されます。スタイルガイドも一貫しています:
...
> 訳: `v-for` には常に `key` を付ける。subtree 内のコンポーネント内部状態を保つために、コンポーネントへの `v-for` では `key` は常に必須となる。
...
> 訳: Vue は当初から、誰もが素早く学べる親しみやすいフレームワークになるという単純なミッションを掲げてきた。ユーザベースが拡大するにつれ、増大する要求に応えるためにスコープも広がり、やがて「Progressive Framework」と呼ぶ形へと進化した。これは、段階的に学習・採用でき、より要求の高いシナリオに取り組むにつれて継続的なサポートも提供する、というフレームワーク像である。
...
3. **状態はできるだけ `ref` に寄せ、どうしても必要なときだけ `reactive`**
   `.value` は負債だが、`reactive` の「置換不能 / destructure で消失 / identity hazard」の方が破綻として大きい。チームのコーディング規約としては `ref` 優先がまず推奨される。

4. **`v-for` には `:key` を常に付ける**
   ドキュメントは「リストレンダーが子コンポーネント state や DOM state を持つときだけ」と書いているが、現実には**最初は state を持たないリストでも後から持つよう変更される**ことが多い。スタイルガイドの "always use key with v-for" に従うのが安全。
...
6. **Template を書く前提に立つ**
   render function や JSX も使えるが、その場合は **2.2 で触れたコンパイラ最適化(静的ホイスト・バインディング種別ヒント等)が効かなくなる代償** と釣り合うかを意識する。普段はテンプレート、動的生成ロジックが本当に必要なときだけ render function に降りる。
...
これらが「なぜダメか」を**機能単位**ではなく**Vueの原理単位**で説明できることが重要です。
...
- 日々書いているパターン(`toRefs` で分解を安全にする、`computed` で派生、`markRaw` で逃がす、…)が**すべて同じ原理の帰結**として見える
- 迷ったときに「Vueはこの問題をどう定義したか?」に立ち返って判断できる
- React / Svelte / Solid / signals 系との比較も「**別の賭けをしている**」という構造で読める

姉妹記事でReactを対で理解しておくとさらに見通しが良くなります。Reactは「状態→ビューを純関数として書かせ、diff で整合性に戻す」、Vueは「値の変化を値自身に監視させ、コンパイラが runtime を縮める」。この二つは**どちらが正解かを問えない別の問題設定**であり、日々の書き方の違いはすべてそこから自然に導かれます。
